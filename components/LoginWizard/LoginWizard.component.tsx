...
export default function LoginWizard({ next }: { next: string }) {
  ...
  return (
    <div className="bg-[#171228] border border-[#2a2450] rounded-3xl p-8 shadow-[0_24px_60px_rgba(0,0,0,0.45)]">
      {/* Mode toggle */}
      <div className="flex gap-1 mb-6 bg-zinc-950/60 border border-zinc-800 rounded-xl p-1">
        {(["signin", "signup"] as const).map((m) => (
          <button
            ...
            className={`flex-1 rounded-lg px-4 py-2 text-sm font-semibold transition-colors ${
              mode === m ? "bg-[#d99a45] text-white" : "text-zinc-400 hover:text-zinc-200"
            }`}
          >
      ...
      {mode === "signin" && signinStep === "otp" && (
        <form onSubmit={submitSigninOtp}>
          <p className="text-sm text-zinc-500 mb-4">{notice}</p>
          <input
            value={code}
      ...
      {mode === "signup" && signupStep === "verify-email" && (
        <form onSubmit={submitEmailCode}>
          ...
          <p className="text-sm text-zinc-500 mb-4">
            Enter the 6-digit code we sent to <span className="text-zinc-300">{email}</span>.
          </p>
          <input
            value={emailCode}
