...
export default async function SiteHeader() {
  ...
  return (
    <>
      <header className="fixed inset-x-0 top-0 z-20 bg-[#081A3A] border-b border-white/10 shadow-[0_6px_20px_rgba(15,23,42,0.18)]">
        <div className="relative max-w-6xl mx-auto px-4 h-16 flex items-center gap-3">
          ...
          <nav className="hidden md:flex absolute left-1/2 -translate-x-1/2 items-center gap-7 text-sm text-[#F8F4E8]/80">
            {links.map((l) => (
              <Link key={l.href} href={l.href} className="hover:text-[#F8F4E8] transition-colors">
                {l.label}
              </Link>
            ))}
          </nav>
      ...
      {/* Spacer — the header above is `fixed`, so it's out of flow; this
          reserves its height so page content doesn't render underneath it.
          Lives here (not in each of the ~10 callers) so nothing else needs
          to change when the header's height changes. */}
